...
import {clone, getInit, getWorkspaceName, isEmpty, numberFormat} from "../../lib/functions";
...
const Index = () => {
    ...
    const setFeedbackOptions = () => {
        if (!isEmpty(feedbacks)) {
            const {
                ...
                followNegative,
                followPositive
            } = feedbacks?.feedbackQuestions[selectedQns] || {};
    ...
    const moveToStep = async (values, action) => {
        ...
        values.feedbackQuestions[selectedQns] = {
            ...values.feedbackQuestions[selectedQns], selectedOptions: categories, star: selectedEmoji
        }
        ...
        if (action === 'prev') {
            ...
            values.feedbackQuestions[selectedQns] = {
                ...values.feedbackQuestions[selectedQns], status: 'completed'
            }
            await setSelectedQns(selectedQns + 1);
    ...
    return (<BodyClassName className={'feedback'}>
            <>
                            ...
                            render={({handleSubmit, values}) => {
                                ...
                                return (<form onSubmit={handleSubmit}>
                                        ...
                                        <div className={'form'}>
                                            ...
                                            <div
                                                className={'d-flex justify-content-between align-items-center  px-4 my-3'}>
                                                <div>
                                                    <h5 className={'m-0'}>Hi! <span className={'text-muted'}
                                                                                    style={{fontWeight: 700}}>{clientname}</span> ,
                                                        your valuable feedback is important for us</h5>
                                                </div>
                                            </div>
                                            ...
                                            <div className={'p-5   border bg-white rounded-4 mt-3'}>
                                                ...
                                                <div className={'d-flex justify-content-between m-auto'}
                                                    ...
                                                    {emojiexp.map((emo, index) => {
                                                        return (<div key={index}
                                                                ...
                                                                name={`${ratingStyle === 'number' ? 'score' : 'emoji'}_${index + 1}.svg`}/>
                                                            </div>)
                                                    })}
                                                </div>
                                                ...
                                                {Boolean(selectedEmoji) && <>
                                                    ...
                                                    {Boolean(setCategories?.length) && categories?.map((option, key) => {
                                                        ...
                                                        return (<div className={`mb-2 rounded-3 bg-white`} key={key}>
                                                    ...
                                                    <div className={'mt-5'}>
                                                        <div className={'mb-2'}>Describe your experience (optional)
                                                        </div>
                                                        <Field name={`feedbackQuestions[${selectedQns}].comments`}>
                                                            {({input, meta}) => (<div className="input-wrp">
